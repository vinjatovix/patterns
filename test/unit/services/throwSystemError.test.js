const { throwSystemError } = require("../../../src/services/throwSystemError");
  ...
  describe("General errors", () => {
    it("should fail with code E31 and status 404 in case of model not found", () => {
      expect(
        (() => {
          try {
            throwSystemError("MODEL_NOT_FOUND", { replace: { id, modelName } });
          } catch (error) {
            return error;
          }
        })()
    ...
    it("should fail with code E33 and status 422 in case of invalid data", () => {
      expect(
        (() => {
          try {
            throwSystemError("DATA_INVALID", { replace: { errors: [message] } });
          } catch (error) {
            return error;
          }
        })()
  ...
  describe("Mongoose Errors", () => {
    it("should fail with code E201 and status 400 in case of mongo validation error", () => {
      expect(
        (() => {
          try {
            throwSystemError("MONGO_VALIDATION_ERROR", { replace: { message } });
          } catch (error) {
            return error;
          }
        })()
    ...
    it("should fail with code E202 and status 409 in case of mongo writing error", () => {
      expect(
        (() => {
          try {
            throwSystemError("MONGO_WRITING_ERROR", { replace: { message, _id: id } });
          } catch (error) {
            return error;
          }
        })()
    ...
    it("should fail with code E203 and status 500 in case of unknown mongo error", () => {
      expect(
        (() => {
          try {
            throwSystemError("MONGO_ERROR", { replace: { message } });
          } catch (error) {
            return error;
          }
        })()
    ...
    it("should fail with code E204 and status 401 in case of mongo unauthorized operation", () => {
      expect(
        (() => {
          try {
            throwSystemError("MONGO_UNAUTHORIZED", { replace: { message } });
          } catch (error) {
            return error;
          }
        })()
